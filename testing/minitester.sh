...
function exec_test()
{
	./minishell -c "$@" 1>$MINISHELL_OUT 2>/dev/null
	./minishell -c "$@" 2>$MINISHELL_ERR 1>/dev/null
	cat -e $MINISHELL_ERR | cut -d ':' -f 2- >$MINISHELL_ERR
	echo "$?">$MINISHELL_EXIT
	bash -c "$@" 1>$BASH_OUT 2>/dev/null
	bash -c "$@" 2>$BASH_ERR 1>/dev/null
	cat -e $BASH_ERR | cut -d ':' -f 3- >$BASH_ERR
	echo "$?">$BASH_EXIT

	if cmp -s "$MINISHELL_OUT" "$BASH_OUT" && cmp -s "$MINISHELL_ERR" "$BASH_ERR" && cmp -s "$MINISHELL_EXIT" "$BASH_EXIT"; then
		printf "$BOLDGREEN[$test_num]\t%s$RESET" "✓ "
		printf "$CYAN [$@] $RESET"
	else
		printf "$BOLDRED[$test_num]\t%s$RESET" "✗ "
		printf "$CYAN [$@] $RESET"
		echo
		if cmp -s "$MINISHELL_OUT" "$BASH_OUT"; then
			printf $BOLDGREEN"STDOUT OK$RESET\n"
		else
			printf $BOLDRED"Minishell STDOUT output: \n%.20s\n" "-----------------------------------------"
			cat -e "$MINISHELL_OUT"
			printf $BOLDRED"\n%.20s$RESET\n" "-----------------------------------------"
			printf $BOLDRED"Bash STDOUT output: \n%.20s\n" "-----------------------------------------"
			cat -e "$BASH_OUT"
			printf $BOLDRED"\n%.20s$RESET\n" "-----------------------------------------"
		fi
		if cmp -s "$MINISHELL_ERR" "$BASH_ERR"; then
			printf $BOLDGREEN"STDERR OK$RESET\n"
		else
			printf $BOLDRED"Minishell STDERR output: \n%.20s\n" "-----------------------------------------"
			cat -e "$MINISHELL_ERR"
			printf $BOLDRED"\n%.20s$RESET\n" "-----------------------------------------"
			printf $BOLDRED"Bash STDERR output: \n%.20s\n" "-----------------------------------------"
			cat -e "$BASH_ERR"
			printf $BOLDRED"\n%.20s$RESET\n" "-----------------------------------------"
		fi
		if cmp -s "$MINISHELL_EXIT" "$BASH_EXIT"; then
			printf $BOLDGREEN"Exit status OK$RESET\n"
		else
			printf $BOLDRED"Minishell exit status: \n%.20s\n" "-----------------------------------------"
			cat -e "$MINISHELL_EXIT"
			printf $BOLDRED"\n%.20s$RESET\n" "-----------------------------------------"
			printf $BOLDRED"Bash exit status: \n%.20s\n" "-----------------------------------------"
			cat -e "$BASH_EXIT"
			printf $BOLDRED"\n%.20s$RESET\n" "-----------------------------------------"
		fi
	fi
...
# ECHO TESTS
printf	"$BOLDYELLOW-----------ECHO TESTS$RESET\n"
...
# CD TESTS
printf	"$BOLDYELLOW-----------CD TESTS$RESET\n"
exec_test 'cd ..'
exec_test 'pwd'
exec_test 'cd /Users'
exec_test 'pwd'
